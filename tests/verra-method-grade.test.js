...
function main() {
  ...
  // 1. VM0047 is not Grade A yet
  const vm0047 = isGradeA(VM0047_DIR);
  assert.equal(vm0047.gradeA, false, 'VM0047 must not be Grade A (3 draft rules, unresolved deps)');
  ...
  assert.equal(vm0047Grade.app_ready, false, 'VM0047 app_ready must be false');
  assert.equal(vm0047Grade.methodology_linked_review_ready, false, 'VM0047 methodology_linked_review_ready must be false');
  ...
  // 4. A method cannot be Grade A with draft_unverified rules
  const vm0007Draft = vm0007.errors.some((e) => e.includes('draft_unverified'));
  assert.ok(vm0007.gradeA === false, 'VM0007 should not claim Grade A');
  // Just verifying it's not Grade A — this is implicit in the assertion above

  // 5. A method cannot be Grade A with active blocked dependencies
  const vm0047Blocked = vm0047.errors.some((e) => e.includes('draft') || e.includes('dependency'));
  assert.ok(vm0047.gradeA === false, 'VM0047 should not claim Grade A');

  // 6. T-SIG historical wording does not count as an active VM0047 blocker
