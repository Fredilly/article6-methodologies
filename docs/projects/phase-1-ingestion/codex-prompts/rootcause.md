...
- `/prompts:rootcause TITLE="validate-json fails on ..."`
- `/prompts:rootcause TITLE="validate-json fails on ..." AREA="ingest" TAGS="schema, ci"`

Steps:
1) Read `docs/projects/phase-1-ingestion/ROOT_CAUSE.md` and follow the format/template in that document.
2) Create a new entry with the one-shot generator:
   - Always run: `npm run -s root-cause:new -- --title "$TITLE"`
   - Only add `--area "$AREA"` if `AREA` is provided (non-empty).
   - Only add `--tags "$TAGS"` if `TAGS` is provided (non-empty).
