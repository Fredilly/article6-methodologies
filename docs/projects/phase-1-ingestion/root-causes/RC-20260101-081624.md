...
- Correct previous-version provenance handling in `scripts/extract-sections.cjs` so `source_ref` uses the previous version (and default previous `tools/.../previous/<v>/tools/source.pdf` layout is supported).
...
- Previous-version `sections.rich.json` provenance must reference `<Code>@<prevVersion>`.
