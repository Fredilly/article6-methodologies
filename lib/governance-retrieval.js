...
function createGovernanceRetriever(rootDir) {
  ...
  function governanceSupportDir(component) {
  ...
  function rule(request) {
    ...
    const found = rules.find((item) => item.id === ruleId || item.stable_id === ruleId || String(item.id || '').endsWith(`.${ruleId}`) || String(item.stable_id || '').endsWith(`.${ruleId}`));
  ...
  function dependency(request) {
    const component = loadComponent(request);
    if (!component) return { found: false, answer: null, uncertainty: 'component_not_found' };
    const filePath = path.join(governanceSupportDir(component), 'dependencies.governance-v1.json');
  ...
  function versionDiff(request) {
    ...
    const filePath = path.join(governanceSupportDir(component), fileName);
