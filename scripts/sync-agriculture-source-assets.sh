...
# For EVERY Agriculture version, copy the authoritative methodology PDF
...
find tools/UNFCCC/Agriculture -type f -name 'source.pdf' | while read -r tools_pdf; do
  rel="${tools_pdf#tools/}"                    # UNFCCC/Agriculture/<Code>/vVV-0/source.pdf
  target="source-assets/${rel}"                # source-assets/UNFCCC/Agriculture/<Code>/vVV-0/source.pdf
  ...
  # but the ingest pipeline expects a canonical source-assets/<Code>/<version>/source.pdf copy.
