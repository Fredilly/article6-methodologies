...
for (const metaPath of walk(MROOT)) {
  ...
  for (const t of tools.slice().sort((a,b)=>String(a.path).localeCompare(String(b.path)))) {
    ...
    if (!fs.existsSync(f) || !fs.statSync(f).isFile()) { issues.push({kind:'missing', path:t.path}); continue; }
    const pointer = readPointer(f);
    let actual = sha256(fs.readFileSync(f));
    if (pointer && pointer.sha256) actual = pointer.sha256;
    if (String(t.sha256) !== actual) issues.push({kind:'mismatch', path:t.path, recorded:String(t.sha256), actual});
  }
